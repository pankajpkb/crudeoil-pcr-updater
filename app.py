...
# 🔥 NEW: Track previous values for difference calculation
previous_put_oi = None
previous_call_oi = None
...
def get_previous_oi_values(sheet):
    """🔥 NEW: Get previous Put and Call OI values from last row"""
    global previous_put_oi, previous_call_oi
    
    try:
        # Get all values in column B (Put OI) and column D (Call OI)
        ...
        if len(put_values) >= 18:
            ...
            for value in reversed(put_values[17:]):  # Start from index 17 (row 18)
                if value and value != '0' and value != '':
                    previous_put_oi = int(value.replace(',', ''))
                    print(f"📊 Previous Put OI: {previous_put_oi:,}")
                    break
            
            # Get the last non-empty value from column D
            for value in reversed(call_values[17:]):  # Start from index 17 (row 18)
                if value and value != '0' and value != '':
                    previous_call_oi = int(value.replace(',', ''))
                    print(f"📊 Previous Call OI: {previous_call_oi:,}")
                    break
                    
    except Exception as e:
        print(f"⚠️ Error getting previous values: {e}")
        previous_put_oi = None
        previous_call_oi = None

def pcr_background_job():
    print("🚀 PCR BACKGROUND JOB STARTED!")
    global last_update_minute, update_in_progress, previous_put_oi, previous_call_oi
    ...
    while True:
        try:
            ...
            # 🔥 NEW: Get previous values for difference calculation
            get_previous_oi_values(sheet)
            
            # 🔥 NEW: Calculate differences
            put_difference = "0"
            call_difference = "0"
            
            if previous_put_oi is not None:
                put_diff_value = put_oi - previous_put_oi
                put_difference = f"{put_diff_value:+,}".replace('+-', '-')  # Format with + or - sign
                print(f"📊 Put OI Difference: {put_difference} (Current: {put_oi:,}, Previous: {previous_put_oi:,})")
            else:
                print("⚠️ No previous Put OI value found for difference calculation")
            
            if previous_call_oi is not None:
                call_diff_value = call_oi - previous_call_oi
                call_difference = f"{call_diff_value:+,}".replace('+-', '-')  # Format with + or - sign
                print(f"📊 Call OI Difference: {call_difference} (Current: {call_oi:,}, Previous: {previous_call_oi:,})")
            else:
                print("⚠️ No previous Call OI value found for difference calculation")
            ...
            # === UPDATED ROW DATA WITH DYNAMIC VALUES + DAY HIGH/LOW + DIFFERENCES ===
            new_row = [
                timestamp, 
                f"{put_oi:,}",           # B - Intraday Put Change OI
                put_difference,           # C - 🔥 NEW: Put OI Difference (Current - Previous)
                f"{call_oi:,}",           # D - Intraday Call Change OI
                call_difference,           # E - 🔥 NEW: Call OI Difference (Current - Previous)
                change_percent, 
                intraday_pcr, 
                "0.00", 
                trend,
            ...
            # 🔥 NEW: Update previous values for next iteration
            previous_put_oi = put_oi
            previous_call_oi = call_oi
            print(f"📊 Updated previous values - Put: {previous_put_oi:,}, Call: {previous_call_oi:,}")
...
@app.route('/update')
def manual_update():
    global update_in_progress, previous_put_oi, previous_call_oi
    try:
        # 🔒 Check if update already in progress
        if update_in_progress:
        ...
        if previous_put_oi is not None:
            ...
            print(f"📊 Put OI Difference: {put_difference}")
        else:
            print("⚠️ No previous Put OI value found")
        
        if previous_call_oi is not None:
            call_diff_value = call_oi - previous_call_oi
            call_difference = f"{call_diff_value:+,}".replace('+-', '-')
            print(f"📊 Call OI Difference: {call_difference}")
        else:
            print("⚠️ No previous Call OI value found")
        ...
        # Updated row with dynamic data + Day High/Low + Differences
        new_row = [
            timestamp, 
            f"{put_oi:,}",           # B - Intraday Put Change OI
            put_difference,           # C - 🔥 NEW: Put OI Difference
            f"{call_oi:,}",           # D - Intraday Call Change OI
            call_difference,           # E - 🔥 NEW: Call OI Difference
            change_percent, 
            intraday_pcr, 
            "0.00", 
            trend,
